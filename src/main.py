...
PIPELINE_FILE = os.path.join(ARTIFACTS_DIR, "pipeline.pkl")
...
if not os.path.exists(MODEL_FILE):
    ...
    input_data = pd.read_csv(os.path.join(DATA_DIR, "input.csv"))
    transformed_input = pipeline.transform(input_data)
    predictions = model.predict(transformed_input)
    input_data["median_house_value"] = predictions
 
    input_data.to_csv(os.path.join(DATA_DIR, "output.csv"), index=False)
